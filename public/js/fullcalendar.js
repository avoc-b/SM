...
DayGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
        ...
        switch(event.status){
            default:
            case 0: iconStatus = 'icon-default'; break;
            case 1: iconStatus = 'icon-primary'; break;
            case 2: iconStatus = 'icon-warning'; break;
            case 3: iconStatus = 'icon-success'; break;
        }
        
		titleHtml =
                        '<i class="icon-circle ' + iconStatus + '"></i>' +
            (event.is_expired ?
                        '<i class="icon-circle icon-expired"></i>' : ''
                        ) +
            (event.is_primary ?
					...
					if (buttonName == 'title') {
						...
						if (buttonClick) {
							...
							if (overrideText) {
                                ...
                                innerHtml = "<span class='glyphicon glyphicon-" + normalIcon + "'></span>";
							}
							else {

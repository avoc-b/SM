 ...
 */
//id, name, startDate
function GanttProjectInfo(projectInfo) {
    this.Id = projectInfo.codeid;
    this.Name = projectInfo.nameid;
    this.Href = projectInfo.href;
    this.StartDate = new Date(projectInfo.date_begin * 1000);
    this.date_begin = projectInfo.date_begin;
    this.date_end = projectInfo.date_end;
    this.Duration = Math.round((projectInfo.date_end - projectInfo.date_begin) / 3600) + 24;
    ...
    var ProjectPercentCompleted = 0;

    // if project has milestones, add milestones to project
    if (projectInfo.milestones.length > 0) {
        $.each(projectInfo.milestones, function(index, milestone) {
            ganttMilestone = new GanttTaskInfo(milestone, true);
            // create tasks for each process in this milestone
            if (milestone.process_list.length > 0) {
                $.each(milestone.process_list, function(index, process) {
                    ganttProcess = new GanttTaskInfo(process, false);
                    ganttMilestone.addChildTask(ganttProcess);
                });
            }
            ganttMilestone.TotalTasks = milestone.process_list.length;
            self.addTask(ganttMilestone);
            self.TotalTasks += milestone.process_list.length + 1;

            ProjectPercentCompleted += ganttMilestone.PercentCompleted;
        });
    }

    if (ProjectPercentCompleted != 0) {
        this.percentCompleted = Math.round(ProjectPercentCompleted / projectInfo.milestones.length);
    }
}
 ...
 * @topic: 0
 */
function GanttTaskInfo(entity, isMilestone) {
    this.entity = entity;
    this.Id = entity.codeid;
    this.Name = entity.nameid;
    this.EST = new Date(entity.date_begin * 1000);
    this.Duration = (entity.date_end - entity.date_begin) / 3600 + 24;
    this.PercentCompleted = entity.percent_completed;
    this.PredecessorTaskId = entity.code_parent_process;
...
function ganttGetDateString(date) {
    return date.getDate().toString() + ' ' + ganttRuMothsArray[date.getMonth()-1];
}
...
GanttProject.prototype.checkWidthProjectNameItem = function()
{
    if (this.projectNameItem.offsetWidth + this.projectNameItem.offsetLeft > this.Chart.maxWidthPanelNames)
    {
        ...
        // hardcoded count of visible chars
        //var pName = this.Project.Id + '. ' + this.Project.Name.substring(0, this.projectNameItem.firstChild.length - countChar - 6);
        //var pName = this.Project.Id + '. ' + this.Project.Name.substring(0, 36);
        //pName += "...";
        this.projectNameItem.innerHTML = this.Project.Id + '. <a href="' + this.Project.Href + '">' + this.Project.Name.substring(0, 36) + '...</a>';
    }
};
/**
...
function GanttChart()
{
    ...
    this.imgs = "/js/gantt/img/";
    this.stylePath = "/js/gantt/dhtmlxgantt.css"; // used in simple printing method getPrintableHTML()

    this.shortMonthNames = ["Янв", "Фев", "Март", "Апр", "Май", "Июнь", "Июль", "Авг", "Сен", "Окт", "Ноя", "Дек"];
    this.monthNames = ["Январь", "Февраль", "Март", "Апрель", "Май", "Июнь", "Июль", "Август", "Сентябрь", "Октябрь", "Ноябрь", "Декабрь"];
    this._useShortMonthNames = false;
 ...
 * @topic: 1
 */
GanttProject.prototype.insertTask = function(entity, isMilestone)
{
    var task = null;
    var _task = null;

    var id = entity.codeid,
            name = entity.nameid,
            EST = new Date(entity.date_begin * 1000),
            Duration = (entity.date_end - entity.date_begin) / 3600 + 24,
            PercentCompleted = entity.percent_completed,
            predecessorTaskId = entity.code_parent_process,
            parentTaskId = entity.code_milestone;
    ...
    if ((parentTaskId) && (parentTaskId != "")) {
        ...
        task = new GanttTaskInfo(entity, isMilestone);
        /*
...
GanttTask.prototype.setPercentCompleted = function(percentCompleted)
{
    ...
    $ganttPercentCompletedContainer = $(this.cTaskItem).find('.ganttTask tr');
    $ganttPercentCompletedContainer.html('');

    if (this.TaskInfo.entity.performers.length > 0) {
        fillOverlayBackground = ' background:' + this.TaskInfo.entity.performers[0].color + ';';
    } else {
        fillOverlayBackground = '';
    }

    if (this.TaskInfo.PercentCompleted != 0) {
        html += '<td width="' + this.TaskInfo.PercentCompleted + '%" height="12px" style="line-height:1px;">' +
                '	<div style="height:12px;' + fillOverlayBackground + '" class="ganttProgressFilled">' +
                '		<div style="height:12px;" class="ganttProgressFilledOverlay"></div>' +
                '	</div>' +
                '</td>';
    }

    if (this.TaskInfo.PercentCompleted != 100) {
        html += '<td width="' + (100 - this.TaskInfo.PercentCompleted) + '%" height="12px" style="line-height:1px;">' +
                '	<div style="height:12px;' + fillOverlayBackground + '" class="ganttProgressEmpty">' +
                '		<div style="height:12px;" class="ganttProgressEmptyOverlay"></div>' +
                '	</div>' +
                '</td>';
    }

    $ganttPercentCompletedContainer.html(html);
    /*
     if ((percentCompleted != 0) && (percentCompleted != 100))
     {
...
GanttChart.prototype.loadData = function(content, isFile, isLocal)
{
    ...
    for (var i = 0; i < this.Project.length; i++)
    {
        ...
        for (var k = 0; k < this.Project[i].ParentTasks.length; k++) {
            /* changed : remove throw error if wrong tast start and end
...
//      var self = this, date = new Date(this.startDate);
...
GanttChart.prototype.createPanelTasks = function()
{
    ...
    // divTasks.style.cssText = "position:relative;width:8976px";
    divTasks.style.cssText = "position:relative;";
    divTasks.style.height = this.contentHeight - 63 + "px";
    divTasks.innerHTML = '<div id="ganttLine" style="position:absolute; left:0; top:0; height:1px; width:100%; background:#900; display:none;"></div>';
...
GanttChart.prototype.createPopUpInfo = function()
{
    ...
    divTaskInfo.className = 'poPupInfo';
...
// console.log(day);
   ...
   // console.log(n);
    ...
    if (day == currentDate.getDate() && date.getMonth() == currentDate.getMonth() && date.getYear() == currentDate.getYear()) {
        $('.ask').append('<div class="ganttTodayLine" style="left:' + ($(newCell).position().left) + 'px"></div>');
    }
...
GanttChart.prototype.checkHeighPanelTasks = function()
{
    if (this.Project.length > 0) {
        var TotalHeight = 0;
        $.each(this.Project, function(index, Project) {
            TotalHeight += Project.TotalTasks * 25 + 25;
        });
        this.contentHeight = TotalHeight + 100;
    } else {
...
GanttChart.prototype.create = function(divId)
{
    ...
    tableControl.style.cssText = "position: relative;";
    // tableControl.className="ask";
    var newRowTblControl = tableControl.insertRow(tableControl.rows.length);
    var newCellTblControl;

// changed: calculate height by number of tasks in all projects
    ...
    this.panelTime.appendChild(this.createPanelTime());
    this.panelTime.style.cssText = "position:relative;height:40px;top:0px;left:1px";

    //Creation panel oData
    this.oData = document.createElement("div");
    this.oData.appendChild(this.createPanelTasks());
    this.oData.style.cssText = "position:relative;height:" + (this.contentHeight - 40) + "px;border-left:#f1f3f1 1px solid";
    ...
    if (this._showTreePanel)
    {
        ...
        this.panelNames.appendChild(this.createPanelNamesTasks());
        this.panelNames.style.cssText = "position:relative;left:0px;top:40px;z-index:5;overflow:hidden;border-left:#f1f3f1 1px solid;border-bottom:#f1f3f1 1px solid;width:300px";
        this.panelNames.className = "asd";
        newCellTblControl.appendChild(this.panelNames);
        newRowTblControl.appendChild(newCellTblControl);
    }
// $(document).ready(function(){
// var CurrentScroll = 0;
// var CurrentScroll2 = 0;
//   $(window).scroll(function(event){

//       var NextScroll = $(this).scrollTop();
//       var NextScroll2 = $(this).scrollLeft();
//       console.log(NextScroll);
//       console.log(NextScroll2);
//       // если скроллит вниз
//       if (NextScroll > CurrentScroll){
//         $('.asd').removeClass('class name2');
//         $('.asd').addClass('class_name');
//       }
//       CurrentScroll = NextScroll;

//        // если скроллит влево
//       if (NextScroll2 > CurrentScroll2){
//         alert(2)
//         $('.asd').removeClass('class name');
//         $('.asd').addClass('class_name2');
//       }
//       CurrentScroll2 = NextScroll2;
//   });
// });
...
//     $(".panelNames").css("top",-n);
// })
// $(document).ready(function(){
//     var top = $('.asd').offset().top - parseFloat($('.asd').css('margin-top').replace(/auto/, 0));
//   $(window).scroll(function(event){
...
GanttTask.prototype.getPopUpInfo = function(object, event)
{
    ...
    //data of task
    var tblInfo = this.Chart.divInfo;
    performers_html = '';

    if (this.TaskInfo.entity.performers.length > 0) {
        performers_html = '<ul class="performers">';
        $.each(this.TaskInfo.entity.performers, function(index, performer) {
            if (index > 4)
                return;
            performers_html += '<li><span class="user-thumb" style="background-color:' + performer.color + ';"><img src="' + performer.avatar + '" title="' + performer.fio + '" rel="tipsy-tooltip" /></span></li>';
        });
        performers_html += '</ul>';
    }

    html =
            '<h3>' + this.TaskInfo.Id + '. ' + this.TaskInfo.Name + '</h3>' +
            performers_html +
            '<ul>' +
            '    <li><span class="title">Дата:</span><span class="value">' + ganttGetDateString(this.TaskInfo.EST) + '</span></li>' +
            '    <li><span class="title">Продолжительность:&nbsp;</span><span class="value">' + this.TaskInfo.Duration + ' часов </span></li>' +
            '    <li class="border"><span class="title">Выполнено:&nbsp;</span><span class="value">' + this.TaskInfo.PercentCompleted + '% </span></li>';

    //show predecessor task
    if (this.predTask) {
        html +=
                '    <li>' +
                '        <span class="title">Предыдущая задача:</span>' +
                '        <span class="value">' + this.TaskInfo.PredecessorTask.Name + '</span>' +
                '    </li>';
    }
    ...
    if (this.TaskInfo.ChildTasks.length != 0) {
        ...
        for (var i = 0; i < this.TaskInfo.ChildTasks.length; i++) {
            tasks += (i == this.TaskInfo.ChildTasks.length - 1) ? ('<li><span class="value">' + this.TaskInfo.ChildTasks[i].Name + '</span></li>') : ('<li><span class="value">' + this.TaskInfo.ChildTasks[i].Name + '</span></li>');
        }

        html +=
                '    <li>' +
                '        <span class="title">Подзадачи:</span>' +
                '        <ul>' +
                tasks +
                '        </ul>' +
                '    </li>';
    ...
    if (this.TaskInfo.ParentTask) {
        html +=
                '    <li><span class="title">Этап:</span><span class="value">' + this.TaskInfo.ParentTask.Name + '</span></li>';
    }

    html += '</ul>';

    tblInfo.innerHTML = html;
 ...
 * @topic: 5
 */
GanttTask.prototype.endMove = function(event)
{
    ...
    if (this.wasMoved && !this.TaskInfo.isMilestone) {
        EllyCore.ajax({
            url: 'index.php?module=process&action=changeDate',
            data: {
                code_process: this.TaskInfo.Id,
                date_begin: est.getDate() + '.' + (est.getMonth() + 1) + '.' + est.getFullYear(),
                duration: Math.round(this.TaskInfo.Duration),
                allday: 1,
            },
            success: function(data) {
                self.TaskInfo.entity = data.process;
            },
            event: event,
        });
    }
...
/** changed: removed shift project item at all
...
GanttProject.prototype.getPopUpInfo = function(object, event)
{
    ...
    tblInfo.innerHTML =
            ...
            '<ul>' +
            '    <li><span class="title">Начало:</span><span class="value">' + ganttGetDateString(this.Project.StartDate) + '</span></li>' +
            '    <li><span class="title">Продолжительность:&nbsp;</span><span class="value">' + Math.round(this.Duration) + ' часов </span></li>' +
            '    <li><span class="title">Выполнено:&nbsp;</span><span class="value">' + this.percentCompleted + '% </span></li>' +
            '</ul>';

    if (posY + this.Chart.divInfo.offsetHeight + 6 > this.Chart.oData.offsetHeight + this.Chart.oData.scrollTop) {
        this.Chart.divInfo.style.top = (posY - this.Chart.divInfo.offsetHeight - 10 - this.Chart.heightTaskItem) + "px";
    } else {
        this.Chart.divInfo.style.top = posY + "px";
    }

    if (this.Chart.divInfo.offsetWidth + posX + 10 > this.Chart.oData.offsetWidth + this.Chart.oData.scrollLeft) {
        this.Chart.divInfo.style.left = posX - (this.Chart.divInfo.offsetWidth + posX + 20 - (this.Chart.oData.offsetWidth + this.Chart.oData.scrollLeft)) + "px";
    } else {
        this.Chart.divInfo.style.left = posX + "px";
    }
    tblInfo.style.display = 'block';
...
GanttTask.prototype.resizeItem = function(event)
{
    if (this.checkResize)
    {
        ...
        if (typeof (this.durationInDays) == 'undefined') {
            this.durationInDays = Math.round(days);
        }

        if (Math.round(days) != days || this.durationInDays == days) {
            return;
        }
...
GanttTask.prototype.resizeTaskItem = function(width)
{
    ...
    if (c)
    {
        c.style.width = parseInt(c.width) * width / 100 + "px";
    }
    ...
    //resize info
    date = new Date(this.TaskInfo.EST);
    if (this.durationInDays > 1) {
        date.setDate(date.getDate() + this.durationInDays - 1)
    }
    this.cTaskItem[0].childNodes[1].firstChild.rows[0].cells[0].innerHTML = date.getDate() + '.' + (date.getMonth() + 1) + '.' + date.getUTCFullYear();
    ...
    for (var i = 0; i < this.Chart.paramShowProject.length; i++) {
        ...
        switch (this.Chart.paramShowProject[i]) {
            ...
            case "StartDate":
                ...
                str += ganttGetDateString(d);
                break;
            case "Duration":
                /*
                 if (str != "")str += delim;
                 str += Math.round(this[this.Chart.paramShowProject[i]]) + "ч";
                 */
                break;
...
GanttProject.prototype.createDescrProject = function()
{
    ...
    divDesc.style.cssText += ";z-index:3;position:absolute;left:" + posX + "px;top:" + this.posY + "px;";
...
GanttProject.prototype.createProjectItem = function()
{
    ...
    tblProjectItem.style.cssText = "border: solid 1px #e89d08;";
...
GanttProject.prototype.createProjectNameItem = function()
{
    ...
    divName.style.cssText = "cursor:pointer;color:#003366;font-weight:bold;font-size:12px;font-family:Tahoma,Arial;white-space:nowrap;height:15px;z-index:1;position:absolute;left:5px;top:" + this.posY + "px;";
    divName.innerHTML = this.Project.Id + '. <a href="' + this.Project.Href + '">' + this.Project.Name + '</a>';
...
GanttProject.prototype.getDuration = function()
{
    ...
    if (this.Project.ParentTasks.length > 0)
    {
        ...
        return Math.round((this.Project.date_end - this.Project.date_begin) / 3600) + 24
    ...
    for (var i = 0; i < this.Chart.paramShowTask.length; i++) {
        ...
        switch (prop) {
            ...
            case "EST":
                if (str != "")
                    str += delim;
                str += ganttGetDateString(propValue);
                break;
            case "S-F":
                if (str != "")
                    str += delim;
...
GanttTask.prototype.getResizeInfo = function()
{
    ...
    {
        this.maxWidthResize = posParentTaskItem + widthParentTaskItem - posTaskItem + 1000;
    }
...
GanttTask.prototype.createTaskItem = function()
{
    ...
    itemControl.style.cssText = 'left:' + this.posX + 'px; top:' + this.posY + 'px;';
    itemControl.className = (this.TaskInfo.isMilestone) ? 'ganttMilestoneContainer' : 'ganttTaskContainer';
    // itemControl.innerHtml +=<a href="#" data-toogle="tooltip" title="Подсказка"></a>;
    // itemControl.title='ad';
    var showLine = function() {
        $('#ganttLine').css('display', 'block').css('top', parseInt(itemControl.style.top) + 18);
    };

    var hideLine = function() {
        $('#ganttLine').css('display', 'none');
    };

    this.addEvent(itemControl, 'mouseover', showLine, false);
    this.addEvent(itemControl, 'mouseout', hideLine, false);

    var divTaskItem = document.createElement("div");
    itemControl.appendChild(divTaskItem);
    divTaskItem.style.className = 'taskItemContainer ' + (this.TaskInfo.isMilestone ? 'ganttMilestone' : 'ganttTask');
    ...
    tblTaskItem.className = (this.TaskInfo.isMilestone) ? 'ganttMilestone' : 'ganttTask';

    if (this.TaskInfo.entity.performers.length > 0 && !this.TaskInfo.isMilestone) {
        tblTaskItem.style.cssText = "border-color:" + this.TaskInfo.entity.performers[0].color;
    }
    ...
    if (this.TaskInfo.PercentCompleted != 0)
    {
        ...
        var imgPr = document.createElement("div");
        imgPr.style.height = this.Chart.heightTaskItem + "px";
        imgPr.className = 'ganttProgressFilled';
        cellTblTask.appendChild(imgPr);
        if (this.TaskInfo.entity.performers.length > 0 && !this.TaskInfo.isMilestone) {
            var imgPrGr = document.createElement("div");
            imgPrGr.style.height = this.Chart.heightTaskItem + "px";
            imgPrGr.className = 'ganttProgressFilledOverlay';
            imgPr.appendChild(imgPrGr);
            imgPr.style.background = this.TaskInfo.entity.performers[0].color;
        }
    }
    ...
    if (this.TaskInfo.PercentCompleted != 100)
    {
        ...
        cellTblTask.style.lineHeight = "1px";
        var imgPrF = document.createElement("div");
        imgPrF.style.height = this.Chart.heightTaskItem + "px";
        imgPrF.className = 'ganttProgressEmpty';
        cellTblTask.appendChild(imgPrF);
        if (this.TaskInfo.entity.performers.length > 0 && !this.TaskInfo.isMilestone) {
            var imgPrGr2 = document.createElement("div");
            imgPrGr2.style.height = this.Chart.heightTaskItem + "px";
            imgPrGr2.className = 'ganttProgressEmptyOverlay';
            imgPrF.appendChild(imgPrGr2);
            imgPrF.style.background = this.TaskInfo.entity.performers[0].color;
        }
    }
    ...
    // divTaskName.title="При редактировании нельзя менять дату на близлежащие задачи";
    ...
    if (this.Chart.isEditable && !this.TaskInfo.isMilestone)
    {
        //Create resize area
        ...
        var prepareMove = function(e) {
            self.addEvent(document, 'mousemove', startMove, true);
            self.addEvent(document, 'mouseup', removeMove, true);
        };

        var removeMove = function(e) {
            document.removeEventListener("mousemove", startMove, true);
            document.removeEventListener("mouseup", removeMove, true);

            if (!self.TaskInfo.isMilestone) {
                var performer_ids = [];
                $code_parent_process = $('#code_parent_process');
                $code_parent_process.html('<option></option>');
                $('#code_parent_process_container').css('display', 'block');
                $.each(self.Project.Project.ParentTasks, function(index, milestone) {
                    if (milestone.ChildTasks.length > 0) {
                        $.each(milestone.ChildTasks, function(index, process) {
                            $code_parent_process.append('<option value="' + process.Id + '">' + process.Name + '</option>');
                        });
                    }
                });

                if (self.TaskInfo.entity.performers.length > 0) {
                    $.each(self.TaskInfo.entity.performers, function(index, performer) {
                        performer_ids.push(performer.codeid);
                    });
                }
                self.TaskInfo.entity['performers[]'] = performer_ids;
                /*
                 EllyCore.showAjaxForm({
                 element: $('#process_form_container'),
                 url: 'index.php?module=process&action=edit',
                 title: 'Редактирование задачи',
                 values: self.TaskInfo.entity,
                 callback: function(){
                 $('#process_date_begin').datepicker('option', 'minDate', null);
                 $('#process_date_begin').datepicker('option', 'maxDate', null);
                 $('#process_date_end').datepicker('option', 'minDate', null);
                 $('#process_date_end').datepicker('option', 'maxDate', null);

                 $('#process_form_container').find('.chzn-select').trigger('liszt:updated');
                 if ( self.childPredTask!=null && self.childPredTask.length > 0 ) {
                 $.each(self.childPredTask, function(index, task) {
                 taskDateStart = new Date(task.TaskInfo.EST);
                 taskDateStart.setDate(taskDateStart.getDate()-1);
                 if ( $('#process_date_end').datepicker('option', 'maxDate')==null || $('#process_date_end').datepicker('option', 'maxDate') < taskDateStart ) {
                 $('#process_date_begin').datepicker('option', 'maxDate', $.datepicker.formatDate('dd.mm.yy', taskDateStart));
                 $('#process_date_end').datepicker('option', 'maxDate', $.datepicker.formatDate('dd.mm.yy', taskDateStart));
                 }
                 });
                 }
                 if ( self.predTask!=null ) {
                 taskDateEnd = new Date(self.predTask.TaskInfo.EST);
                 taskDateEnd.setHours(taskDateEnd.getHours()+self.predTask.TaskInfo.Duration);
                 if ( $('#process_date_end').datepicker('option', 'minDate')==null || $('#process_date_end').datepicker('option', 'minDate') > taskDateEnd ) {
                 $('#process_date_begin').datepicker('option', 'minDate', $.datepicker.formatDate('dd.mm.yy', taskDateEnd));
                 $('#process_date_end').datepicker('option', 'minDate', $.datepicker.formatDate('dd.mm.yy', taskDateEnd));
                 }
                 }
                 },
                 });
                 */
                $.colorbox({transition: 'none', initialWidth: 910, initialHeight: 575, href: 'index.php?module=widget&action=process&codeid=' + self.TaskInfo.Id, ajax: true});
            }
        };
                    ...
                    if (self.wasResized) {
                        ...
                        if (!self.TaskInfo.isMilestone) {
                            EllyCore.ajax({
                                url: 'index.php?module=process&action=changeDate',
                                data: {
                                    code_process: self.TaskInfo.Id,
                                    date_begin: self.TaskInfo.EST.getDate() + '.' + (self.TaskInfo.EST.getMonth() + 1) + '.' + self.TaskInfo.EST.getFullYear(),
                                    duration: self.TaskInfo.Duration,
                                    allday: 1,
                                },
                                success: function(data) {
                                    self.TaskInfo.entity = data.process;
        ...
        divStatus.className = 'taskStatus';

        if (this.TaskInfo.entity.status_accepted == 1) {
            perform = (this.TaskInfo.entity.status_perform == 1) ? '<i class="color-icons status_yellow_co" title="Выполненно"></i>' : '<i class="color-icons status_blue_co" title="Принято"></i>';
        } else {
            perform = '<i class="color-icons status_gray_co" title="Не принято"></i>';
        }

        checked = (this.TaskInfo.entity.status_checked == 1) ? '<i class="color-icons status_green_co" title="Проверенно"></i>' : '<i class="color-icons status_gray_co" title="Не проверенно"></i>';
        priority = (this.TaskInfo.entity.priority == 1) ? '<i class="color-icons priority_co" title="Важная задача"></i>' : '';

        divStatus.innerHTML = priority + perform + checked;
        divTaskName.appendChild(divStatus);

        this.addEvent(divMove, 'mousedown', prepareMove, false);
        this.addEvent(divStatus, 'mousedown', prepareMove, false);
...
GanttTask.prototype.createTaskNameItem = function(hasChildren) {
    ...
    divName.style.cssText = 'top:' + this.posY + 'px;';

    if (!this.TaskInfo.isMilestone) {
        divName.style.cssText += 'left:30px;';
    }

    if (this.TaskInfo.entity.performers.length > 0) {
        divName.style.cssText += 'color:' + this.TaskInfo.entity.performers[0].color;
    }

    var showLine = function() {
        $('#ganttLine').css('display', 'block').css('top', parseInt(divName.style.top) + 18);
    };
    ...
    this.addEvent(divName, 'mouseover', showLine, false);
    this.addEvent(divName, 'mouseout', hideLine, false);

    // add class if task is milestone
    divName.className = (this.TaskInfo.isMilestone) ? 'taskNameItem taskNameMilestone' : 'taskNameItem';
    ...
    if (this.TaskInfo.entity.performers.length > 0 && this.TaskInfo.isMilestone) {
        ...
        performers_html += '</ul>';
        divName.innerHTML += performers_html;
    }
    if (this.TaskInfo.isMilestone) {
        divName.innerHTML += '<a href="javascript://" class="btnAddProcess" data-toogle="tooltip" title="Нельзя создавать вторую родительскую задачу,создаются только подзадачи" code_milestone="' + this.TaskInfo.Id + '" code_project="' + this.Project.Project.Id + '"><span class="color-icons bullet_add_1_co"></span></a>';
        var aName = document.createElement('a');
        aName.href = 'javascript://';
        aName.className = 'nameContainer';

        aName.innerHTML = (this.TaskInfo.isMilestone) ? this.TaskInfo.entity.order + '. ' + this.TaskInfo.Name : this.TaskInfo.Name;

        aName.onclick = function() {
            treeImg = document.getElementById('ganttPlusMinus_' + self.TaskInfo.Id);
            treeImg.click();
        }
    } else {
        aName = document.createElement('div');
        aName.className = 'nameContainer';
        aName.innerHTML = this.TaskInfo.Name;
    }
...
GanttTask.prototype.setStatus = function(process) {
    this.TaskInfo.entity = process;

    if (process.status_accepted == 1) {
        perform = (process.status_perform == 1) ? '<i class="color-icons status_yellow_co" title="Выполненно"></i>' : '<i class="color-icons status_blue_co" title="Принято"></i>';
    } else {
        perform = '<i class="color-icons status_gray_co" title="Не принято"></i>';
    }

    checked = (process.status_checked == 1) ? '<i class="color-icons status_green_co" title="Проверенно"></i>' : '<i class="color-icons status_gray_co" title="Не проверенно"></i>';
    priority = (process.priority == 1) ? '<i class="color-icons priority_co" title="Важная задача"></i>' : '';
    $(this.cTaskItem).find('.taskStatus').html(priority + perform + checked);
    this.setPercentCompleted(process.percent_completed);
}

/**
 * changed: hardcoded length of visible chars in task name
...
GanttTask.prototype.checkWidthTaskNameItem = function() {
    ...
    $taskNameItem = $(this.cTaskNameItem[0]);

    if (!this.TaskInfo.isMilestone && this.TaskInfo.Name.length > 44) {

        $taskNameItem.find('div.nameContainer').html(this.TaskInfo.Name.substring(0, 44) + '...');

    } else if (this.TaskInfo.isMilestone && this.TaskInfo.Name.length > 30) {

        $taskNameItem.find('a.nameContainer').html(this.TaskInfo.Name.substring(0, 30) + '...');
    ...
    /*
     ...
     //tName += "...";
     this.cTaskNameItem[0].innerHTML = this.TaskInfo.Name.substring(0, 36) + '...';
...
GanttTask.prototype.create = function()
{
    ...
    if (this.Chart.panelNames)
    {
        ...
        if (parentTask)
        {
            this.cTaskNameItem[0].style.left = parseInt(this.parentTask.cTaskNameItem[0].style.left) + "px";
...
GanttTask.prototype.createTreeImg = function()
{
    ...
    treeImg.id = 'ganttPlusMinus_' + this.TaskInfo.Id;
    ...
    treeImg.style.cssText = "cursor:pointer;left:" + (parseInt(this.cTaskNameItem[0].style.left) - 12) + "px;top:" + (parseInt(this.cTaskNameItem[0].style.top) + 1) + "px;z-index:12;position:absolute;";
...
GanttError.prototype._init = function()
{
    ...
    this._errors[10] = "Reduce duration of task(%0),при выборе родительской задачи нельзя у подзадачи дату меньше поставить";
...
contextMenu.prototype._init = function()
{
    ...
    var self = this;
    var arrItems = [];
    var tab1 = this.createTab(1, 'Добавить задачу', 't', false, this);
    return;
    ...
    tab9.addItem(9, "Insert", document.createElement("input"), "button",
            function() {
                try {
                    ...
                    if (tab9.object.insertTask(id, name, est, duration, pc, predTaskId, parentTaskId))
                        tab9.hide();
    ...
    tab11.addItem(6, "Insert", document.createElement("input"), "button",
            function() {
                try {
                    ...
                    if (pr.insertTask(id, name, est, duration, pc, predTaskId, parentTaskId))
                        tab11.hide();
